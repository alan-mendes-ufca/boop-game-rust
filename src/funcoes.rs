...
pub fn fluxo_jogo(tabuleiro: &mut Tabuleiro, jogada: &Jogada, jogador: Jogador, maos: &mut Maos) {
    ...
    let jogada_fechou_trio = !crate::graduar::trios_de_gatinhos(tabuleiro, jogador).is_empty();

    if !jogada_fechou_trio {
        crate::jogada::boop_nas_oito_direcoes(
            tabuleiro,
            jogada.linha,
            jogada.coluna,
            jogada.peca,
    ...
    crate::graduar::graduar(tabuleiro, jogador, maos);
    crate::graduar::graduar(tabuleiro, jogador.oponente(), maos);
}
